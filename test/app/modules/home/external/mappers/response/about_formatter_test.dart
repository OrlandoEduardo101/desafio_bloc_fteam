...
import 'package:desafio_bloc_fteam/app/modules/home/external/formatters/response/about_formatter.dart';
import 'package:flutter_test/flutter_test.dart';
import '../../../shared/jsons/jsons.dart';

main() {
  var mapper = AboutFormatter();
  group('check keys', () {
    test('must check json without erros', () {
      expect(mapper.checkJson(jsonDecode(about)), completes);
    });
  ...
  group('from map teste', () {
    test('must return assignmentsResponse without erros', () async {
      expect(mapper.fromMap(jsonDecode(about)), completes);
      expect(await mapper.fromMap(jsonDecode(about)),
          isA<AboutEntity>());
    });
  ...
  group('to map test', () {
    test('must return a map', () {
      expect(
          mapper.toMap(AboutEntity(href: 'href')), isA<Map>());
    });
  });
}
