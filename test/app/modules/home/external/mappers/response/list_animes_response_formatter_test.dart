...
main() {
  var mapper = ListAnimesResponseFormatter();
  group('check keys', () {
    test('must check json without erros', () {
      expect(mapper.checkJson({'listAnimes': [jsonDecode(listAnimes)]}), completes);
    });
  ...
  group('from map teste', () {
    test('must return assignmentsResponse without erros', () async {
      expect(mapper.fromMap({'listAnimes': [jsonDecode(listAnimes)]}), completes);
      expect(await mapper.fromMap({'listAnimes': [jsonDecode(listAnimes)]}),
    ...
    test('must return MapperErrorActivityError from error type', () async {
      ...
      await mapper
          .fromMap({'listAnimes': 0}).catchError(
              // ignore: avoid_types_on_closure_parameters
              (e) async {
        error = e.message ?? '';
        return ListAnimesResponse();
  ...
  group('to map test', () {
    test('must return a map', () {
      expect(
          mapper.toMap(ListAnimesResponse(listAnimes: [])), isA<Map>());
    });
  });
}
